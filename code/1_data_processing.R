...
#Temperature
temperature <- read.csv(paste0(dir.data,'weekly_temperature.csv'), header=T)[,-c(1,2)]
chelonia.strandings <- merge(chelonia.strandings, temperature, by=c('day','month', 'year'))
caretta.strandings <- merge(caretta.strandings, temperature, by=c('day','month', 'year'))
