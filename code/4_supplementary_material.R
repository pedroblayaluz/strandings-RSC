...
temperature.month <- temperature %>%
  filter(year >= 1996) %>%
  ggplot(aes(x=month,y=sst)) +
  geom_jitter(aes(color=sst),size=.3, alpha=0.5, stat='unique') +
...
ggsave(filename = 'strandings_monthly_variation.jpg',
       path=dir.sup,
       plot=monthly.strandings,
       width=18,
       height=9,
       units='cm',
       dpi=320)
ggsave(filename = 'covariates.jpg',
       path=dir.sup,
       plot=covariates,
       width=18,
       height=13,
